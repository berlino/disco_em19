...
class Hypergraph(nn.Module):
    ...
    def forward(self,
        ...
        self.span_cache = span_vectors  # sharing with fine classifiers
        ...
        diff = (loss_vec_relu - loss_vec).max().item()
        # if diff > 1e-4: # arould singel precision
        if output["loss"].item() < 0:
            # pdb.set_trace()
    ...
    def decode(self,
        ...
        self.span_cache = IX_vectors  # sharing with fine classifiers
            ...
            for j in range(sent_len):
                for k in range(self.config.label_size):
                    ...
                    def recur_find(s):
                        ...
                        if v_A.item() > 0 and v_B.item() > 0:
                            best_ends.append(s)
                            return v_A + v_B, best_ends
                        elif v_B.item() <= 0 and v_A.item() >= v_B.item():
                            return v_A, [s]
                        else:
                    ...
                    if TX_scores[i, j, k].item() < seq_score.item():
                        for end in end_list:
